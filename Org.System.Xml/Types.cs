...
namespace Org.System.Xml
{
  ...
  public struct QName
  {
    ...
    public static QName Checked(string localName, string nsUri) {
      ...
      if (Uri.IsWellFormedUriString(nsUri, UriKind.Absolute))
        return new QName(localName, nsUri);
      else {
        string msg = Resources.GetString(RsId.InvalidUri);
        throw new ArgumentException(String.Format(msg, nsUri));
      }
    }
    ...
    /// <summary>Checks if the <see cref="QName"/> instance is well-formed.</summary>
    public bool IsWellFormed {
      get { 
        return XmlChars.IsNcName(localName) && (nsUri == String.Empty ||
          Uri.IsWellFormedUriString(nsUri, UriKind.Absolute));
      }
    }
    ...
    /// <summary>Converts a local name string to a <see cref="QName"/>.</summary>
    /// <param name="localName">String value to be interpreted as a local name
    /// that is not in any namespace.</param>
    /// <returns>New <see cref="QName"/> instance initialized from local name,
    /// and an empty namespace URI.</returns>
    public static explicit operator QName(string localName) {
      return new QName(localName);
    }
